...
void Solution::changeDemands(float PD){
    ...
    for(Truck *t: problemInstance->trucks){ //problema, quedan camiones
        ...
        for ( int i =0 ; i < totalLiterBenefitByType.size() ; ++i) {
            ...
            if ((beta <= choiceProbability) or (stoi(to_string(choiceProbability)) == 100)) {
                newDemands[i] += (int)(PD*t->getTotalCapacity()); //TODO funciona(?)
...
vector<int> Solution::convertMilk(bool print) { //Si print es true va a agregar toda la info al vector, leche por tipo, y los tipos que se transformaron si hubo transformacion // cuando es falso se agrega solo la info de las cantidades correspondientes de cada tipo.
    vector<int> blending;

    int aux(0);
    for (int i: problemInstance->qualities) { //cantidad de cada tipo requerida.
        if (i == 0) {
    ...
    for (Route *r: this->routes) { //Se suma la cantidad de leche de cada tipo que viene en cada camión.
        ...
        //cout << "Se sumaron : " << liters << " a los litros de tipo: " << r->getTypeIndex() << endl;
    }

    vector<int> recollectedMilk = litersxtype; //this->recollected;
                        ...
                        if (diff >= 0) {
                            if (i == j) {
                                if (print) {
                                    ...
                                    //cout << recollectedMilk[j] << " de leche " << i << " se usa como leche " << j << " en la planta" << endl;
                                }
                                ...
                                if (print) {
                                    ...
                                    blending.push_back(j);
                                    blending.push_back(i);
                                    //cout << dda[i] << " de leche* " << j << " se usa como leche " << i << " en la planta" << endl;
                                }
                                totalMilk[i] += dda[i];
                                break;
                            }
